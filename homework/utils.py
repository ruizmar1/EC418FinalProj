...
from controller import PPOPolicy
from controller import compute_gae
from controller import ppo_update
...
class PyTux:
    ...
    # Roll out function for training PPO
    def rollout_train(self, track, controller, PPO_policy, optimizer, epochs, planner=None, max_frames=1000, verbose=False, data_callback=None):
        if self.k is not None and self.k.config.track == track:
            self.k.restart()
            self.k.step()
        else:
        ...
        # Added variables to be used by PPO algorithm, inspired by henanmemeda RL-Adventure-2/ppo
        log_probs= []
        values = []
        states = []
        actions = []
        ...
        # this is where we are doing a roll out
        for t in range(max_frames):
            state.update()
            track.update()
            ...
            # calling controler function to get action object from given aimpoint and current policy
            action, value, dist, action_tensor = controller(aim_point_image, current_vel, PPO_policy)

            #dealing with log prob
            ...
            values.append(value)
            state_vec = [aim_point_image[0], aim_point_image[1], current_vel]
            action_vec = [action.steer, action.acceleration, action.brake, action.drift, action.nitro]
            states.append(state_vec)
            actions.append(action_vec)
            rewards.append(reward)
            # appended masks earlier on
        ...
        # if we have done 1000 steps and still have not completed the track, lets take ONE more step through the environment
        if done == 0:
            state.update()
            track.update()
            ...
            if np.isclose(kart.overall_distance / track.length, 1.0, atol=2e-3):
                ...
                current_vel = np.linalg.norm(kart.velocity)
                _, next_value, _ , _ = controller(aim_point_image, current_vel, PPO_policy)

                #calculating GAE
                returns = compute_gae(next_value, rewards, mask, values)
